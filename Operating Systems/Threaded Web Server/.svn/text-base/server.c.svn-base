...
int main (int argc, char* argv[]) {
	...
	// no cache if invalid specified
	if( config.cache_enabled != CACHE_OLDEST 
		|| config.cache_enabled != CACHE_LRU)  {
...
void* worker_main (void* arg) {
	...
	while (1) {
		...
		//
		// NOTE: get_cache is where LRU re-queuing happens
		//
...
void* dispatcher_main (void* arg) {
	...
	while (1) {
		...
		// atomically get a request number
		req->req_num = server_stats_incr_req_num( &stats );
		
		fprintf( stderr, "Dispatcher: putting request in queue\n" );
...
// =============================================================================
//
//    main() init/setup/shutdown functions
//
// =============================================================================



// create request and prefetch queues, and the cache.
int create_queues() {
...
// setup a signal handler to shutdown all theads
int setup_sighandler() {
...
int create_threads() {
	...
	for (i = 0; i < config.num_dispatch; i++) {
		...
		*arg = thread_id++;
		...
		pthread_detach( dispatch_threads[i] );
	}
